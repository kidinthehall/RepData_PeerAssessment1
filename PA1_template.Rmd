...
```{r computesummary, echo=TRUE, warning=FALSE}
options("scipen"=100)
...
For the total number of steps taken per day, the mean is `r datesummean` and the median is `r datesummedian`.
...
The max number of average steps is taken at the **`r maxinv`** interval, around **`r maxhour`:`r maxmin`**, and peaks at **`r maxsteps` steps**.
...
For the total number of steps taken per day when adjusted for the NAs, the mean has changed from **`r datesummean` to `r datesum3mean`** and the median changed from **`r datesummedian` to `r datesum3median`**.
